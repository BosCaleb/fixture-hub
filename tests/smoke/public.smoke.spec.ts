...
test.describe('Tournament Hub public smoke tests', () => {
  test('@smoke home page loads with sport choices', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /tournament manager/i })).toBeVisible();
  ...
  test('@smoke netball selection routes to tournaments page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/netball\/tournaments$/);
    await expect(page.getByRole('heading', { name: /netball tournaments/i })).toBeVisible();
